...
\end{figure}
\section{Results}
We found out the from the line regulation experiment with the Zener Diode that the \textbf{breakdown voltage} is $\pmb{V_b = 4.34V}$ when
the input voltage is $\pmb{V_{min} = 10.51V}$. 

On Day 2, The breakdown voltage for the load regulation experiment with $R_c = 1k\Omega$ is $V_b = 6.44V$.

For the line and load regulation with IC 7805, the \textbf{regulated voltage} is $\pmb{V_R = 5.11V}$, which 
requires a minimum input voltage of $\pmb{V_{clamp} = 6.48V}$.
